	...
	loop {
		...
		match &cmd.to_ascii_uppercase()[..] {
			...
			"EHLO" => {
				...
				let repl = format!("250-{} looking good {}\r\n", conn.config.myname, conn.addr.ip());
...
fn format_received(helo: &String, revname: &String, ipaddr: IpAddr, myname: &String, proto: &String,
		   sessid: &String, rcpt: &String, timestamp: &String) -> String {
	return format!(
r"Received: from {} ({} [{}])
...
fn deliver_local(conn: &Connection, data: String) -> Result<()> {
	...
	for rcpt in &conn.rcpts {
		...
		let recv = format_received(&helo, &revname, ipaddr, &host, &proto.to_string(), &conn.sessid, &rcpt, &timestamp);
...
fn save_for_relay(conn: &Connection, data: String) -> Result<()> {
	...
	let timestamp = now.to_rfc2822();
	for rcpt in &conn.rcpts {
		let mut mesg = format_received(&helo, &revname, ipaddr, &host, &proto.to_string(), &conn.sessid, &rcpt, &timestamp);
...
pub fn after_switch<R: BufRead, W: Write>(config: &Config, from: &String, rcpt: &String, data: String,
					 mut r: R, mut w: W) -> Result<()> {
	let g = format!("ehlo {}\r\n", config.myname);
	w.write(g.as_bytes())?;
	w.flush()?;
	loop {
...
fn client_send<R: BufRead, W: Write>(from: &String, rcpt: &String, data: String,
					 mut r: R, mut w: W) -> Result<()> {
