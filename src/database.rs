...
pub fn save_mesg_to_db(from: &str, rcpt: &str, mesg: String, due: i64, tries: i64) {
    let conn = open_database();
...
pub fn delete_mesg(mesgid: i64) {
    let conn = open_database();
    if let Err(e) = conn.execute("delete from mesgs where mesgid = ?", params![mesgid]) {
...
pub fn requeue_mesg(mesgid: i64, tries: i64) {
    ...
    let conn = open_database();
    if let Err(e) = conn.execute(
        "update mesgs set due = ?, tries = ? where mesgid = ?",
